var canvas = document.getElementById('canvas'), 
	...
	bangouElement = document.getElementById('bangou'),
	bangouModeElementNA = document.getElementById('bangouModeNA'),
	bangouModeElementRomaji = document.getElementById('bangouModeRomaji'),
	bangouModeElementHiragana = document.getElementById('bangouModeHiragana'),
	bangouModeElementKanji = document.getElementById('bangouModeKanji'),
	bangouMode = 'na',
...
function updateBangou() {
	if (!bangouElement || !abacus) {
		return;
	}
	var values = [];
	for (var i = 0; i < abacus.numberOfRods; i++) {
		values.push(abacus.rods[i].value);
	}
	var firstNonZero = 0;
	while (firstNonZero < values.length - 1 && values[firstNonZero] === 0) {
		firstNonZero++;
	}
		...
		for (var i = 0; i < abacus.numberOfRods && !found; i++) {
			...
			for(var j = 0; j < currentRod.beads.length && !found; j++) {
				...
				if (context.isPointInPath(loc.x, loc.y)) {
					found = true;
					if (soundToggle && soundToggle.checked && beadSound) {
						beadSound.play();
					}
...
function clickedBead(bead) {
	if (bead.heaven) {
		if (bead.active) {
			...
			var allLowerActive = true;
			for (var i = 1; i <= 4; i++) {
				var lowerBead = getBead(bead.rod, false, i);
				if (!lowerBead.active) {
					allLowerActive = false;
					break;
				}
			}
			if (allLowerActive) {
...
abacus = new Abacus(parseInt(numberOfRodsElement.value, 10), 'normal', frameColor, false, false);
canvas.width = abacus.width + 2 * LEFT_MARGIN;
...
if (bangouModeElementNA) {
	bangouModeElementNA.onclick = function() {
		bangouMode = 'na';
		updateBangou();
	};
}
if (bangouModeElementRomaji) {
	bangouModeElementRomaji.onclick = function() {
		bangouMode = 'romaji';
		updateBangou();
	};
}
if (bangouModeElementHiragana) {
	bangouModeElementHiragana.onclick = function() {
		bangouMode = 'hiragana';
		updateBangou();
	};
}
if (bangouModeElementKanji) {
	bangouModeElementKanji.onclick = function() {
		bangouMode = 'kanji';
		updateBangou();
	};
}
...
numberOfRodsIndex = numberOfRodsIndex !== null ? parseInt(numberOfRodsIndex, 10) : null;
if (numberOfRodsIndex !== null && !Number.isNaN(numberOfRodsIndex)) {
	numberOfRodsElement.selectedIndex = numberOfRodsIndex;
}
numberOfRodsElement.onchange();
